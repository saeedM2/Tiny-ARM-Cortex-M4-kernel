...
if not os.path.exists('logs/'):
    os.makedirs('logs/')
logging.basicConfig(filename='logs/debugMsg.log', format='%(asctime)s %(levelname)-8s %(message)s', level=logging.INFO, datefmt='%Y-%m-%d %H:%M:%S')
...
def exec_command(commandString, logCriticalEnable = True, logAllEnable = False, logTimeEnable = False):
    fh = None
